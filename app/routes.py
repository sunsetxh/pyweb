...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Comgratulations, you are now a registered user!')
        return redirect(url_for('login'))
    return render_template('register.html', title='Registered', form=form)
...
def unfollow(username):
    ...
    db.session.commit()
    flash('You arc not following {}.'.format(username))
    return redirect(url_for('user', username=username))
